...
from shfa.abstract import views
...
def get_serializer(model: models.Model, fields: Callable[[models.Model], List[str]] = get_fields, depth: int = 0) -> serializers.ModelSerializer:
    ...
    class SHFABaseSerializer(serializers.ModelSerializer):

        class Meta:
            model = None 

    SHFABaseSerializer.Meta.model = model
    SHFABaseSerializer.Meta.fields = fields(model)
    SHFABaseSerializer.Meta.depth  = depth
    SHFABaseSerializer.Meta.ref_name = model._meta.model_name

    return SHFABaseSerializer
...
def build_contact_form_endpoint(name: None):
